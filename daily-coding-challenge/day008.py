...
# and the one 'A' leaf. The 'A' leaf causes
...
# We can start off by first writing a function that
# checks whether a tree is unival or not.
# Then, perhaps we could use this to count up all the nodes in the tree.

# To check whether a tree is a unival tree,
# we must check that every node in the tree has the same value.
# To start off, we could define an is_unival function that
# takes in a root to a tree.
...
# And then our function that counts the number of subtrees could
# simply use that function:
...
# we're evaluating each node in its subtree again as well.
...
# Also returns number of unival subtrees,
# and whether it is itself a unival subtree.
