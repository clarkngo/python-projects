...
class Solution:
    def missing_lowest_positive(self, arr: List) -> int:
        length: int = len(arr)
        smallest: int = min(filter(lambda x: x > 0,arr))    # filter negative numbers
        sequence: List = []
        for i in range(smallest, length+1):         # create a list of correct sequence of numbers
            sequence.append(i)
        return list(set(sequence)- set(arr))[0]     # return an element of "sequence" list that is missing in "arr" list
...
class Test(unittest.TestCase):
    def test(self):
        self.assertEqual(a.missing_lowest_positive(self, [3, 4, -1, 1]), 2)
    def test2(self):
        self.assertEqual(a.missing_lowest_positive(self, [1, 2, 0]), 3)
