...
# We can see that the recursive structure is as follows:

# If string starts with zero, then there's no valid encoding.
# If the string's length is less than or equal to 1,
# there is only 1 encoding.
# If the first two digits form a number k that is less than or equal to 26,
# we can recursively count the number of encodings assuming
# we pick k as a letter.
# We can also pick the first digit as a letter and
# count the number of encodings with this assumption.
...
def num_encodings(s):
    ...
    return total
# However, this solution is not very efficient.
# Every branch calls itself recursively twice,
# so our runtime is O(2n). We can do better by using dynamic programming.

# All the following code does is repeat the same computation
# as above except starting from the base case and building up the solution.
